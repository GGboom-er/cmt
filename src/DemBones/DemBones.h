...
/** @mainpage Overview
        Main elements:
        - @ref DemBones : base class with the core solver using relative bone transformations
   DemBones::m
        - @ref DemBonesExt : extended class to handle hierarchical skeleton with local
   rotations/translations and bind matrices
        - DemBones/MatBlocks.h: macros to access sub-blocks of packing transformation/position
   matrices for convenience

        Include DemBones/DemBonesExt.h (or DemBones/DemBones.h) with optional DemBones/MatBlocks.h
   then follow these steps to use the library:
        -# Load required data in the base class:
                - Rest shapes: DemBones::u, DemBones::fv, DemBones::nV
                - Sequence: DemBones::v, DemBones::nF, DemBones::fStart, DemBones::subjectID,
   DemBones::nS
        ...
        -# [@c optional] Set parameters in the base class:
                ...
                - DemBones::nWeightsIters, DemBones::nnz, DemBones::weightsSmooth,
   DemBones::weightsSmoothStep, DemBones::weightEps
        -# [@c optional] Setup extended class:
                - Load data: DemBonesExt::parent, DemBonesExt::preMulInv, DemBonesExt::rotOrder,
   DemBonesExt::bind
                - Set paramter: DemBonesExt::bindUpdate
        -# [@c optional] Override callback functions (cb...) in the base class @ref DemBones
        -# Call decomposition function DemBones::compute(), DemBones::computeWeights(),
   DemBones::computeTranformations(), or DemBones::init()
...
/** @class DemBones DemBones.h "DemBones/DemBones.h"
        ...
        @details Setup the required data, parameters, and call either compute(), computeWeights(),
   computeTranformations(), or init().

        Callback functions and read-only values can be used to report progress: cbInitSplitBegin(),
   cbInitSplitEnd(), cbIterBegin(), cbIterEnd(), cbWeightsBegin(), cbWeightsEnd(),
   cbTranformationsBegin(), cbTransformationsEnd(), cbTransformationsIterBegin(),
        cbTransformationsIterEnd(), cbWeightsIterBegin(), cbWeightsIterEnd(), rmse(), #iter,
   #iterTransformations, #iterWeights.

        @b _Scalar is the floating-point data type. @b _AniMeshScalar is the floating-point data
   type of mesh sequence #v.
*/
...
class DemBones {
 public:
  ...
  //! [@c parameter] Number of bone transformations update iterations per global iteration, @c
  //! default = 5
  int nTransIters;
  ...
  DemBones()
      : nIters(30),
        nInitIters(10),
        nTransIters(5),
        transAffine(_Scalar(10)),
        transAffineNorm(_Scalar(4)),
        nWeightsIters(3),
        nnz(8),
  ...
  //! Start frame indices, @c size = #nS+1, #fStart(@p s), #fStart(@p s+1) are data frames for
  //! subject @p s
  Eigen::VectorXi fStart;
  //! Subject index of the frame, @c size = #nF, #subjectID(@p k)=@p s, where #fStart(@p s) <= @p k
  //! < #fStart(<tt>s</tt>+1)
  Eigen::VectorXi subjectID;

  //! Geometry at the rest poses, @c size = [3*#nS, #nV], #u.@a col(@p i).@a segment(3*@p s, 3) is
  //! the rest pose of vertex @p i of subject @p s
  MatrixX u;

  //! Skinning weights, @c size = [#nB, #nV], #w.@a col(@p i) are the skinning weights of vertex @p
  //! i, #w(@p j, @p i) is the influence of bone @p j to vertex @p i
  SparseMatrix w;

  /** @brief Bone transformations, @c size = [4*#nF*4, 4*#nB], #m.@a blk4(@p k, @p j) is the 4*4
     relative transformation matrix of bone @p j at frame @p k
          @details Note that the transformations are relative, that is #m.@a blk4(@p k, @p j) brings
     the global transformation of bone @p j from the rest pose to the pose at frame @p k.
  */
  MatrixX m;
  MatrixX origM;

  //! Animated mesh sequence, @c size = [3*#nF, #nV], #v.@a col(@p i).@a segment(3*@p k, 3) is the
  //! position of vertex @p i at frame @p k
  Eigen::Matrix<_AniMeshScalar, Eigen::Dynamic, Eigen::Dynamic> v;

  //! Mesh topology, @c size=[<tt>number of polygons</tt>], #fv[@p p] is the vector of vertex
  //! indices of polygon @p p
  std::vector<std::vector<int>> fv;

  //! [<tt>zero indexed</tt>, <tt>read only</tt>] Current global iteration number that can be used
  //! for callback functions
  const int& iter;

  //! [<tt>zero indexed</tt>, <tt>read only</tt>] Current bone transformations update iteration
  //! number that can be used for callback functions
  const int& iterTransformations;

  //! [<tt>zero indexed</tt>, <tt>read only</tt>] Current weights update iteration number that can
  //! be used for callback functions
  ...
  /** @brief Initialize missing skinning weights and/or bone transformations
          @details Depending on the status of #w and #m, this function will:
                  ...
                  - Only one in #w or #m is missing (zero size): initialize missing matrix, i.e. #w
     (or #m)
                  - Both #w and #m are missing (zero size): initialize both with rigid skinning
     using approximately #nB bones, i.e. values of #w are 0 or 1. LBG-VQ clustering is peformed
     using mesh sequence #v, rest pose geometries #u and topology #fv.
                  @b Note: as the initialization does not use exactly #nB bones, the value of #nB
     could be changed when both #w and #m are missing.

          This function is called at the begining of every compute update functions as a safeguard.
  */
  bool init() {
    if (modelSize < 0)
      modelSize = sqrt((u - (u.rowwise().sum() / nV).replicate(1, nV)).squaredNorm() / nV / nS);
  ...
  /** @brief Update bone transformations by running #nTransIters iterations with #transAffine and
     #transAffineNorm regularizers
...
#pragma omp parallel for
        ...
        for (int j = 0; j < nB; j++) {
          ...
          for (int it = uuT.outerIdx(j); it < uuT.outerIdx(j + 1); it++)
            if (uuT.innerIdx(it) != j)
              qpT -= m.blk4(k, uuT.innerIdx(it)) * uuT.val.blk4(subjectID(k), it);
          qpT2m(qpT, k, j);
        }
  ...
  /** @brief Update skinning weights by running #nWeightsIters iterations with #weightsSmooth and
     #weightsSmoothStep regularizers
          @details Required input data:
                  - Rest shapes: #u, #fv, #nV
                  - Sequence: #v, #nF, #fStart, #subjectID, #nS
                  - Number of bones: #nB
...
#pragma omp critical
  ...
  /** @brief Skinning decomposition by #nIters iterations of alternative updating weights and bone
     transformations
          @details Required input data:
                  - Rest shapes: #u, #fv, #nV
                  - Sequence: #v, #nF, #fStart, #subjectID, #nS
    ...
    for (int i = 0; i < nV; i++) {
      ...
      for (int k = 0; k < nF; k++) {
        ...
        ei += (mki.template topLeftCorner<3, 3>() * u.vec3(subjectID(k), i) +
               mki.template topRightCorner<3, 1>() - v.vec3(k, i).template cast<_Scalar>())
                  .squaredNorm();
      }
#pragma omp atomic
      e += ei;
    ...
    if (_qpT(3, 3) != 0) {
      ...
      Eigen::JacobiSVD<Matrix3> svd(
          qpT.template topLeftCorner<3, 3>() -
              qpT.template topRightCorner<3, 1>() * qpT.template bottomLeftCorner<1, 3>(),
      ...
      m.transVec(k, j) = qpT.template topRightCorner<3, 1>() -
...
#pragma omp atomic
  ...
  struct TripletLess {
    bool operator()(const Triplet& t1, const Triplet& t2) { return t1.value() > t2.value(); }
  };
...
#pragma omp critical
    ...
    std::priority_queue<Triplet, std::vector<Triplet, Eigen::aligned_allocator<Triplet>>,
                        TripletLess>
        heap;
    for (int j = 0; j < nB; j++)
...
#pragma omp parallel for
    for (int k = 0; k < nF; k++) {
      ...
      for (int i = 0; i < nV; i++)
        if (label(i) != -1)
          qpT.blk4(0, label(i)) += Vector4(v.vec3(k, i).template cast<_Scalar>().homogeneous()) *
...
#pragma omp parallel for
  ...
  //! vuT.blk4(k, j) = \sum_{i=0}^{nV-1}  w(j, i)*v.vec3(k, i).homogeneous()*u.vec3(subjectID(k),
  //! i).homogeneous()^T
  MatrixX vuT;
...
#pragma omp parallel for
    for (int k = 0; k < nF; k++) {
      ...
      for (int i = 0; i < nV; i++)
        for (typename SparseMatrix::InnerIterator it(w, i); it; ++it) {
          Matrix4 tmp = Vector4(v.vec3(k, i).template cast<_Scalar>().homogeneous()) *
                        u.vec3(subjectID(k), i).homogeneous().transpose();
  ...
  //! uuT is a sparse block matrix, uuT(j, k).block<4, 4>(s*4, 0) = \sum{i=0}{nV-1} w(j, i)*w(k,
...
#pragma omp parallel for
    for (int i = 0; i < nV; i++)
      for (typename SparseMatrix::InnerIterator it(w, i); it; ++it)
        for (typename SparseMatrix::InnerIterator jt(w, i); jt; ++jt) pos(it.row(), jt.row()) = 1;
...
#pragma omp atomic
  ...
  //! \sum_{k=fStart(s)}^{fStart(s+1)-1} m.block<3, 4>(k*4, i*4)^T*m.block<3, 4>(k*4, j*4)
  MatrixX mTm;
    ...
    for (int p = 0; p < nPairs; p++) {
      ...
      for (int k = 0; k < nF; k++)
        mTm.blk4(subjectID(k) * nB + i, j) +=
            m.blk4(k, i).template topRows<3>().transpose() * m.blk4(k, j).template topRows<3>();
      if (i != j)
        for (int s = 0; s < nS; s++) mTm.blk4(s * nB + j, i) = mTm.blk4(s * nB + i, j);
    }
  }

  //! aTb.col(i) is the A^Tb for vertex i, where A.size = (3*nF, nB), A.col(j).segment<3>(f*3) is
  //! the transformed position of vertex i by bone j at frame f, b = v.col(i).
  MatrixX aTb;
        ...
        if ((aTb(j, i) == 0) && (ws(j, i) > weightEps))
          for (int k = 0; k < nF; k++)
            aTb(j, i) += v.vec3(k, i).template cast<_Scalar>().dot(
                m.blk4(k, j).template topRows<3>() * u.vec3(subjectID(k), i).homogeneous());
  }
...
#pragma omp parallel for
  ...
  /** Pre-compute aTa for weights update on one vertex
          @param i is the vertex index.
          @param aTa is the by-reference output of A^TA for vertex i, where A.size = (3*nF, nB),
     A.col(j).segment<3>(f*3) is the transformed position of vertex i by bone j at frame f.
  */
  void compute_aTa(int i, MatrixX& aTa) {
    ...
    for (int j1 = 0; j1 < nB; j1++)
      for (int j2 = j1; j2 < nB; j2++) {
        for (int s = 0; s < nS; s++)
          aTa(j1, j2) += u.vec3(s, i).homogeneous().dot(mTm.blk4(s * nB + j1, j2) *
