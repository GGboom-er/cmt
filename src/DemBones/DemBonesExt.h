...
/**  @class DemBonesExt DemBonesExt.h "DemBones/DemBonesExt.h"
        @brief Extended class to handle hierarchical skeleton with local rotations/translations and
   bind matrices

        @details Call computeRTB() to get local rotations/translations and bind matrices after
   skinning decomposition is done and other data is set.
...
class DemBonesExt : public DemBones<_Scalar, _AniMeshScalar> {
 public:
  ...
  //! Timestamps for bone transformations #m, [@c size] = #nS, #fTime(@p k) is the timestamp of
  //! frame @p k
  ...
  //! Parent bone index, [@c size] = #nB, #parent(@p j) is the index of parent bone of @p j,
  //! #parent(@p j) = -1 if @p j has no parent.
  Eigen::VectorXi parent;

  //! Original bind pre-matrix, [@c size] = [4*#nS, 4*#nB], #bind.@a block(4*@p s, 4*@p j, 4, 4) is
  //! the global bind matrix of bone @p j on subject @p s at the rest pose
  MatrixX bind;

  //! Inverse pre-multiplication matrices, [@c size] = [4*#nS, 4*#nB], #preMulInv.@a block(4*@p s,
  //! 4*@p j, 4, 4) is the inverse of pre-local transformation of bone @p j on subject @p s
  MatrixX preMulInv;

  //! Rotation order, [@c size] = [3*#nS, #nB], #rotOrder.@a col(@p j).@a segment<3>(3*@p s) is the
  //! rotation order of bone @p j on subject @p s, 0=@c X, 1=@c Y, 2=@c Z, e.g. {0, 1, 2} is @c XYZ
  //! order
  Eigen::MatrixXi rotOrder;

  //! Bind transformation update, 0=keep original, 1=set translations to p-norm centroids (using
  //! #transAffineNorm) and rotations to identity
  int bindUpdate;
  ...
  /** @brief Local rotations, translations and global bind matrices of a subject
          @details Required all data in the base class: #u, #fv, #nV, #v, #nF, #fStart, #subjectID,
     #nS, #m, #w, #nB
          ...
          @param[out] lr is the [3*@p nFr, #nB] by-reference output local rotations, @p lr.@a col(@p
     j).segment<3>(3*@p k) is the (@c rx, @c ry, @c rz) of bone @p j at frame @p k
          @param[out] lt is the [3*@p nFr, #nB] by-reference output local translations, @p lt.@a
     col(@p j).segment<3>(3*@p k) is the (@c tx, @c ty, @c tz) of bone @p j at frame @p k
          @param[out] gb is the [4, 4*#nB] by-reference output global bind matrices, @p gb.@a
     block(0, 4*@p j, 4, 4) is the bind matrix of bone j
          @param[out] lbr is the [3, #nB] by-reference output local rotations at bind pose @p lbr.@a
     col(@p j).segment<3>(3*@p k) is the (@c rx, @c ry, @c rz) of bone @p j
          @param[out] lbt is the [3, #nB] by-reference output local translations at bind pose, @p
     lbt.@a col(@p j).segment<3>(3**3k) is the (@c tx, @c ty, @c tz) of bone @p j
          @param[in] degreeRot=true will output rotations in degree, otherwise output in radian
  */
  void computeRTB(int s, MatrixX& lr, MatrixX& lt, MatrixX& gb, MatrixX& lbr, MatrixX& lbt,
                  bool degreeRot = true) {
    computeBind(s, gb);
...
#pragma omp parallel for
    for (int j = 0; j < nB; j++) {
      ...
      if (parent(j) == -1)
        ...
        lb = preMulInv.blk4(s, j) * gb.blk4(0, parent(j)).inverse() * gb.blk4(0, j);
      ...
      for (int k = 0; k < nFs; k++) {
        if (parent(j) == -1)
          ...
          lm = preMulInv.blk4(s, j) *
               (m.blk4(k + fStart(s), parent(j)) * gb.blk4(0, parent(j))).inverse() *
 ...
 private:
  /** p-norm centroids (using #transAffineNorm) and rotations to identity
          ...
          @param b is the [4, 4*#nB] by-reference output global bind matrices, #b.#a block(0, 4*@p
     j, 4, 4) is the bind matrix of bone @p j
  */
  ...
  /** Global bind pose
          @param s is the subject index
          @param bindUpdate is the type of bind pose update, 0=keep original, 1=set translations to
     p-norm centroids (using #transAffineNorm) and rotations to identity
          @param b is the the [4, 4*#nB] by-reference output global bind matrices, #b.#a block(0,
  ...
  */
  ...
  /** Euler angles from rotation matrix
          ...
          @param curRot is the input current Euler angles, it is also the by-reference output closet
     Euler angles correspond to @p rMat
